...
def preprocess():
    ...
    return train_data, train_label, validation_data, validation_label, test_data, test_label, np.nonzero(mask_toDel)
...
def nnObjFunction(params, *args):
    ...
    yl = np.zeros((total_inputs,n_class))
    ...
    sum_1 = np.add(parta, np.multiply((1-yl), np.log((1-ol))))

    sum_1 = np.divide(np.sum(sum_1),(-1)*total_inputs)

    # print("NEXT")
    partsum = np.sum(np.square(w1)) + np.sum(np.square(w2))

    sum_2 = (partsum) * np.divide(lambdaval,(2*total_inputs))
    ...
    grad_w2 = np.divide(np.add(derivate2,np.multiply(lambdaval,w2)),total_inputs)
    ...
    grad_w1 = np.add(w1_1,np.multiply(lambdaval,w1))/total_inputs
...
ff = open('output_check_4_30.txt','w')
...
if  max_training < comp_training:
    ...
    obj = [selected_features, n_hidden, w1, w2, lambdaval] #n_hidden,  w_1, lambda ]
    pickle.dump(obj, open('params_training.pickle_25_50', 'wb'))
...
if  max_validation < comp_validation:
    ...
    #[w1,n_hidden, w_1, w_2, lambda]
    obj = [selected_features, n_hidden, w1, w2, lambdaval] #n_hidden,  w_1, lambda ]
    pickle.dump(obj, open('params_validation_25_50.pickle', 'wb'))
...
if  max_test < comp_test:
    ...
    #[w1,n_hidden, w_1, w_2, lambda]
    obj = [selected_features, n_hidden, w1, w2, lambdaval] #n_hidden,  w_1, lambda ]
    pickle.dump(obj, open('params_test.pickle_25_50', 'wb'))           
